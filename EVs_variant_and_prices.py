...
def collect_evs_variant_and_prices(url):

    all_data_list = []  # Create a list to store the table data
    html = requests.get(url).content
    soup = BeautifulSoup(html, 'lxml')

    # Find the scooter name
    name = soup.find('h1', class_='sectiom_block_style__Heading-sc-drgjgi-1 kXKqYB')
    name_text = name.get_text()

    # Date when data scrap
    recorded_date = str(datetime.date.today())


    # EV scooter variants and its prices
    table = soup.find('tbody', class_='table_style__CTbody-sc-1qo50z6-4 fDhzGn')
    if table:
        table_rows = table.find_all('tr', class_='table_style__CTr-sc-1qo50z6-2 kIirDc')
        for table_row in table_rows:
            table_data_list = table_row.find_all('td', class_='table_style__CTd-sc-1qo50z6-5 caVOSP vartd-name')
            for table_data in table_data_list:
                variant_name = table_data.find('a', href=True).get_text()
                variant_price = table_row.find('td',
                                               class_='table_style__CTd-sc-1qo50z6-5 caVOSP vartd-price').get_text()
                all_data_list.append({'name': name_text, 'variant': variant_name, 'variant_price': variant_price,
...
for url in urls:
    data = collect_evs_variant_and_prices(url)
    all_data.extend(data)

my_data = pd.json_normalize(all_data)

with engine.connect() as connection:
    my_data.to_sql('variant_and_prices', con=connection, if_exists='append', index=False, schema='dbo')
