...
def collect_reviewer_data(url):
    ...
    all_reviews_box = soup.find_all('li', class_='border-b pd-b pd-t')

    # The first 3 reviews

    for review in all_reviews_box[:3]:
        reviewer_name_element = review.find('div',
                                            class_='review_style__CUserName-sc-ar1d7i-3 ePiEXh lineHtSmall font-12 color-54')
        reviewer_name = reviewer_name_element.find(string=re.compile(r'.*')).strip() if reviewer_name_element else None

        review_date_element = review.find('div', class_='color-37 font-12')
        review_date = review_date_element.get_text() if review_date_element else None

        review_text_element = review.find('div', class_='model_overview_style__CMaxPara-sc-16p2w99-1 jqJmdU')
        review_text = review_text_element.get_text() if review_text_element else None

        # Store the individual review data in a dictionary
        review_data = {
            'name': dictionary_of_details['name'],
            'recorded_date' : dictionary_of_details['recorded_date'],
...
all_reviews_data = []  # To store individual reviewer data

for url in urls:
    reviews = collect_reviewer_data(url)
    all_reviews_data.extend(reviews)


review_data = pd.json_normalize(all_reviews_data)

# Truncate reviewer_text to a maximum length
max_text_length = 1000              # limit to 1000 characters
review_data['reviewer_text'] = review_data['reviewer_text'].str[:max_text_length]


with engine.connect() as connection:
   review_data.to_sql('reviewers_data', con=connection, if_exists='append', index=False, schema='dbo')
